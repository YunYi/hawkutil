...
namespace Hawk
{
	...
	UInt32 HawkOSOperator::GetTickCount()
	{
		...
		iSysMs = curTm.tv_sec*1000 + curTm.tv_usec/1000; 
#endif
	...
	UInt32  HawkOSOperator::CalcCrc(const UChar* pData, UInt32 iSize)
	{
		UInt32 iCRC = 0;
	...
	{
		if (rename(sOldName.c_str(),sNewName.c_str()) == HAWK_OK)
			return true;
